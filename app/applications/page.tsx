...
export default function ApplicationsPage() {
  ...
  const handleDownloadClick = () => {
    const selectedItems = filteredData.filter((_, index) => checkedItems[index]);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 m-2 w-full gap-2">
          {filteredData.map(({ name, image }, index) => (
            <div key={index} className="flex flex-row items-center p-1 h-20 rounded-lg dark:bg-default-100 shadow">
              <Checkbox
                className="ml-2 mr-2"
              ...
              />
              <h2 className="ml-4 text-foreground/90 text-lg font-medium">{name}</h2>
            </div>
          ))}
        </div>
      )}
